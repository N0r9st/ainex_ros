        ...
        try:
            ...
            for color_info in self.detect_info:  # 遍历颜色列表
                ...
                if self.image_process_size != last_process_size:
                    image_resize = cv2.resize(image_gb, tuple(self.image_process_size), interpolation=cv2.INTER_NEAREST)  # 图像缩放, 加快图像处理速度, 不能太小，否则会丢失大量细节
                ...
                if color_info.detect_type == 'line':  # 巡线检测
                    ...
                    for roi in line_roi:
                        ...
                        if max_contour is not None:
                            ...
                            cv2.drawContours(image_draw, [corners], -1, (0, 255, 255), 3, cv2.LINE_AA)  # 绘制矩形轮廓
                            center_x = x
                            center_y = y
                            break
                    ...
                    if center_x != 0:
                        ...
                        cv2.circle(image_draw, (center_x, center_y), 3, draw_color, -1)  # 画出中心点
                else:
                    roi = color_info.roi
                    ...
                    if color_info.detect_type not in ['cross', 'intersection']:
                        ...
                        if max_contour is not None:
                            if color_info.detect_type == 'rect':
                                ...
                                draw_color = self.cal_mean_bgr(image_draw, x, y)
                                cv2.circle(image_draw, (x, y), radius, draw_color, 3)  # 画圆
