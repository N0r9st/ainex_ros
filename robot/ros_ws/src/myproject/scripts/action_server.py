...
class ActionServer:
    ...
    def servo_control_callback(self, msg):
        ...
        try:
            ...
            if servo_data:
                self.servo_controller.set_servos_position(int(time_ms), [servo_data])
                self.action_status_pub.publish(f"COMPLETED: servo_control {msg.data}")
            else:
                rospy.logwarn("No valid servo data found in command.")

        except json.JSONDecodeError:
            rospy.logerr(f"Failed to decode JSON from servo_control message: {msg.data}")
            self.action_status_pub.publish(f"ERROR: Invalid JSON in servo_control: {msg.data}")
        except Exception as e:
            rospy.logerr(f"Error in servo_control_callback: {e}")
            self.action_status_pub.publish(f"ERROR: servo_control failed: {e}")
