...
class HiwonderServoController:
    ...
    def set_servo_position(self, servo_id, position, duration=None):
        ...
        if servo_id == 11:
            if position < 385:
                self.save_servo_data('11:' + str(position), 'data1')
                position = 385
            if position > 570:
                self.save_servo_data('11:' + str(position), 'data1')
                position = 570
        if servo_id == 12:
            if position < 430:
                self.save_servo_data('12:' + str(position), 'data2')
                position = 430
            if position > 615:
                self.save_servo_data('12:' + str(position), 'data2')
                position = 615
    ...
    def set_servos_position(self, duration, args):
        ...
        for arg in args[0]:
            ...
            if _id == 11:
                if _pulse < 385:
                    self.save_servo_data('11:' + str(_pulse), 'data1')
                    _pulse = 385
                if _pulse > 570:
                    self.save_servo_data('11:' + str(_pulse), 'data1')
                    _pulse = 570
            if _id == 12:
                if _pulse < 430:
                    self.save_servo_data('12:' + str(_pulse), 'data2')
                    _pulse = 430
                if _pulse > 615:
                    self.save_servo_data('12:' + str(_pulse), 'data2')
                    _pulse = 615
