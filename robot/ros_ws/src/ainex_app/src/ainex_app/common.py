...
class ColorPicker:
    ...
    def get_color(self, image, result_image):
        h, w = image.shape[:2]
        ...
        if 0 <= x < w and 0 <= y < h:
            ...
            result_image = cv2.circle(result_image, (x, y), self.count, (0, 255, 255), 5)
...
class Heart:
    ...
    def heartbeat_srv_callback(self, msg: SetBoolRequest):
        ...
        if msg.data:
            self.heartbeat_stamp = rospy.get_time() + self.timeout
        else:
            self.heartbeat_stamp = 0
        return SetBoolResponse(success=True)

    def heartbeat_timeout_check(self, timer_event):
        if self.heartbeat_stamp != 0 and self.heartbeat_stamp < rospy.get_time():
